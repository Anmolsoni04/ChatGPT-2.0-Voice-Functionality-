...
open_api_key = "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"
openai.api_key = open_api_key

@login_required(login_url='signin') #this is the python decorator
def index(request):
    today = date.today()
...
def getValue(request):
    ...
    #check if the message is a voice input 
    if message.startswith("/voice"):
        message = message.replace("/voice", "").strip() #remove the "/voice" prefix from the message
